...
static wifi_handler_ap_credentials_t apCredential_list[ CONFIG_MAX_ENTRIES_ON_AP_CRED_LIST ] = { false };

static wifi_config_t wifi_config = {
    .sta = {
        .ssid = CONFIG_WIFI_DEFAULT_SSID,
        .password = CONFIG_WIFI_DEFAULT_PASS,
...
static void wifiHandlerAPTask( void* taskParmPtr );
...
int32_t wifiHandlerStart( uint8_t nChannels, wifi_promiscuous_cb_t wifiPacketHandler ) 
{
    ...
    if ( 0 != snifferFilter.filter_mask ) {
...
static void wifiHandlerAPTask( void* taskParmPtr ) 
{
    ...
    for (;;) {
        ...
        if ( ( xBits & WIFI_DISCONNECTED ) && !( xBits & WIFI_CONNECTING ) && ( xBits & WIFI_SCAN_STOP ) && !( xBits & WIFI_SCAN_START ) ) {
            if ( ++retryConCnt > CONFIG_WIFI_RECONNECT_TRIES ) {
                retryConCnt = 0;
...
int wifiHandlerGetAPIndexFromListbyMAC( uint8_t *mac2find )
{
    ...
    for ( i = 0 ; i < CONFIG_MAX_ENTRIES_ON_AP_CRED_LIST; i++ ) {
        imac = apCredential_list[ i ].macaddr;
        if ( 0 == memcmp( imac, nullentry, MAC_ADDR_LENGTH ) ) { 
            index = -1; /*end of the list reached*/
            break;
        }
        if ( 0 == memcmp( imac, mac2find, MAC_ADDR_LENGTH ) ) { 
            index = i;
            break;
        }
    }
...
int wifiHandlerAPCredentialListInsertSSDI( int index, char *ssid, int8_t ap_rssid )
{
    ...
    if ( index < CONFIG_MAX_ENTRIES_ON_AP_CRED_LIST ) {
        if ( 0 == strlen( apCredential_list[ index ].ssid ) ) { /*only the first time, when the entry is empty*/
            strncpy( apCredential_list[ index ].ssid, ssid, CONFIG_MAX_SSID_NAME_LENGTH  );
        }
...
int wifiHandlerSetBestAPbyList( void )
{
    ...
    qsort( apCredential_list,  CONFIG_MAX_ENTRIES_ON_AP_CRED_LIST, sizeof(wifi_handler_ap_credentials_t), wifiHandlerRSSICmpFcn );
    bestSignalAp = apCredential_list[ 0 ]; /*AP with the best signal on top*/

    if ( ( bestSignalAp.valid ) && ( strlen( bestSignalAp.ssid ) > 0u ) ) {
        strncpy( (char*)wifi_config.sta.ssid, bestSignalAp.ssid, CONFIG_MAX_SSID_NAME_LENGTH );
        strncpy( (char*)wifi_config.sta.password, bestSignalAp.pwd, CONFIG_MAX_CRED_PWD_LENGTH );
        ESP_LOGI( TAG, "Connecting to AP [ %s ] with rssi = %d ...", bestSignalAp.ssid , bestSignalAp.rssi);
        return 0;
    }
    else {
        strncpy( (char*)wifi_config.sta.ssid, CONFIG_WIFI_DEFAULT_SSID, CONFIG_MAX_SSID_NAME_LENGTH );
        strncpy( (char*)wifi_config.sta.password, CONFIG_WIFI_DEFAULT_PASS, CONFIG_MAX_CRED_PWD_LENGTH );
...
int wifiHandlerAPCredentialListStore( wifi_handler_ap_credentials_t *list )
{
    ...
    #if ( CONFIG_WIFI_PERSISTENT_CREDENTIALS == 1 )
        ret = setDataBlockRawToNvs( "apcredlist" , list, CONFIG_MAX_ENTRIES_ON_AP_CRED_LIST*sizeof(wifi_handler_ap_credentials_t) );
    #endif
    if ( ESP_OK == ret ) {
...
int wifiHandlerAPCredentialListLoad( void )
{
    int32_t ret = ESP_ERR_NOT_FOUND;
    #if ( CONFIG_WIFI_PERSISTENT_CREDENTIALS == 1 )
        ret = getDataBlockRawFromNvs( "apcredlist", apCredential_list, CONFIG_MAX_ENTRIES_ON_AP_CRED_LIST*sizeof(wifi_handler_ap_credentials_t) );
    #endif
    if ( ESP_OK == ret ) {
        ...
        if ( 0 == memcmp( &apCredential_list, &emptycheck, sizeof(emptycheck) ) ) {
            ...
            memset( &apCredential_list, 0x00, CONFIG_MAX_ENTRIES_ON_AP_CRED_LIST*sizeof(wifi_handler_ap_credentials_t) );
        }
