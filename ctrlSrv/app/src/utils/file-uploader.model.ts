...
const ACCEPTED_FILE_TYPES = ['image/png', 'image/jpeg'] as const;

type AcceptedFileTypes = typeof ACCEPTED_FILE_TYPES[number];

const getFilesObservable = (input: HTMLInputElement) =>
...
export const getBase64: (file: File) => Observable<string> = (file: File) => {
	...
	return fromEvent(fileReader, 'load').pipe(map(() => fileReader.result as string));
};
