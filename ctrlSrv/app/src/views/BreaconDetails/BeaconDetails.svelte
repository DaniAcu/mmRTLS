<script lang="ts">
  ...
  import type { Marker } from "../../streams/marker.types";
  import Dialog from "../../components/Dialog/Dialog.svelte";
  import { markerSubject } from "../../streams/markers-interactions";

  export let beacon: Marker<Beacon> | null = null;
...
<Dialog isVisible={!!beacon} on:close={onClose}>
  ...
  <section class="flex items-center gap-4 py-8">
    ...
    <ul class="leading-loose flex-1">
      ...
      <li>
        <strong><abbr title="">TSSI</abbr>:</strong>
        <span>{beacon?.data.tssi}</span>
      </li>
      <li>
        <strong>Position:</strong>
        <span>x: {beacon?.data.x}, y: {beacon?.data.y}</span>
      </li>
    </ul>
  </section>
</Dialog>